...
**Status:** Design (rev 5 — dual-gate round 4 folded) — pending user approval
**Date:** 2026-08-23
...
> - **Rev 5 (this):** round-4 re-review (codex BLOCK / independent SHIP-WITH-CHANGES) — both gates confirmed the security core empirically on live Compose v5.1.2 (recursive `.env` expansion leak reproduced; `--profile tls` a rc=0 no-op against a pre-Slice-5 compose; `up --wait` on the one-shot exits 0). No Critical. Folds: **(Important)** restore/update proxy-up ran the chown-skipping `--no-deps` on a possibly fresh root-owned ACME volume → now runs `proxy-init` standalone first; the two-bucket `composeArgs` left `pull`/`exec` to fall through → now an explicit **three-way** split (start=gated / containment=always / everything-else incl. `pull`=no profile, so `install`'s `compose pull` never fetches proxy images on a non-TLS deploy or in air-gapped registries); **`tls disable`** now reaps the proxy idempotently **before** consulting `.env` (§4.3↔§6.2 contradiction removed). **(Minor)** `proxy-init` cap-hardened (`cap_drop:[ALL]`+`cap_add:[CHOWN]`+`no-new-privileges`); restore/update proxy-up gated on `opts.WriteBreadcrumb` so auto-rollback never issues it; strict DNS validator runs before `BuildBaseURL`; newer→older downgrade caveat added to README/man; the pre-Slice-5 no-op recorded.
...
```yaml
  ...
  proxy-init:
    ...
    profiles: ["tls"]
    command: ["sh", "-c", "chown -R 1001:1001 /srv/acme"]
    volumes:
...
1. **`compose rm -sf proxy` first, unconditionally** — before consulting `.env` state. Containment always carries `--profile tls` (§4.3), so this idempotently reaps a running proxy **even when `.env` reads disabled** (a corrupt/partial-edited `.env`, or an `enable` that half-completed). Tolerate `no such service: proxy` (rc=1) from a pre-Slice-5 on-disk compose as success — there is simply nothing to reap.
...
- **restore/update with TLS enabled** brings the proxy back up as a **non-gating, bounded** step, **forward-path only and gated on `opts.WriteBreadcrumb==true`**: the shared `restoreEngine` is called by standalone restore (`restore.go:74`, `WriteBreadcrumb:true`) **and** by update auto-rollback (`update.go:113`, `WriteBreadcrumb:false`), so gating on the breadcrumb flag (or placing the step in the restore-command wrapper) keeps the **rollback path from ever issuing a proxy-up**. The step **restores the ACME-ownership invariant**: `--no-deps` skips `proxy-init` (verified on Compose v5.1.2), so on a **fresh/recreated (root-owned) `mathion_acme` volume** (new-host restore or post-`--purge`) the uid-1001 proxy could not write `/srv/acme`; therefore it runs the chown one-shot **standalone first** — `up -d --wait proxy-init` (no `depends_on`, `network_mode: none`, so it stands up alone under the active profile; **`--wait` is required** — it blocks until `service_completed_successfully`, whereas a plain `up -d` would return once the one-shot is merely *started* and let the proxy race the chown) — **then** `up -d proxy --pull never --no-deps` (keeping `--no-deps` on the proxy so app/db stay undisturbed), each under a short independent timeout with errors demoted to a warning. Hermetic tests: the **rollback** path (`WriteBreadcrumb:false`) issues **no** proxy-up **nor** proxy-init; a **fresh root-owned `mathion_acme`** case asserts `proxy-init` runs before `proxy`; and a Runner whose proxy-up call **blocks until context cancellation** must not delay the app gate or the rollback (assert the timeout cancels it, not merely that an eventual error is demoted).
...
- `cli/cmd/restore.go` and `cli/cmd/update.go` — when TLS is enabled, bring the proxy up as a **separate, non-gating, bounded** step **after** the app-gated `up --wait app`, **gated on `opts.WriteBreadcrumb==true`** (or placed in the restore-command wrapper) so the shared `restoreEngine`'s auto-rollback call (`update.go:113`, `WriteBreadcrumb:false`) issues **no** proxy-up. The step runs the chown one-shot **standalone first** (`up -d --wait proxy-init` — no deps, `network_mode: none`; `--wait` blocks until it completes) so a fresh root-owned `mathion_acme` volume (new-host / post-`--purge`) is chowned before **`up -d proxy --pull never --no-deps`** (each under a short independent timeout, errors demoted). Tests: TLS-enabled restore/update runs `proxy-init` then restarts the proxy; the rollback path runs neither; a blocking proxy-up call never delays the app gate or the rollback.
