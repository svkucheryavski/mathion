...
**Date:** 2026-05-29
**Status:** Design (post 5×6+codex×10 reviewer pass — rev 17 fixes 1 Minor from codex round 16; round 16 verdict was YES, plan-ready). Rev 17 narrows the malformed-percent example precision: the rev-16 status/narrative listed `?next=%E0%A4` alongside `?next=%` as examples of malformed input that would throw `URIError`. Codex round 16 caught that `URLSearchParams.get` substitutes a replacement character for incomplete UTF-8 byte sequences BEFORE the rawNext value reaches Login.svelte, so `?next=%E0%A4` never actually exercises the bug. Only truncated-percent inputs like `?next=%` (no following hex digits) survive URLSearchParams' lenient decoder verbatim, then throw inside `decodeURIComponent`. Rev 17 keeps `?next=%` as the canonical Login-decode example everywhere and drops the `%E0%A4` example from the Login-decode discussion. The `/%E0%A4` example INSIDE the safeNext code comment (line ~700) stays — that example is about a path arriving AT safeNext (e.g., via `?next=%2F%25E0%25A4` which URLSearchParams decodes to `/%E0%A4`), where the new `decodeURI(u.pathname)` guard correctly catches it.

**Earlier (rev 16, codex round 15 — 1 Important + 1 Minor):** (1) `decodeURIComponent(rawNext)` AFTER `URLSearchParams.get` is a redundant double-decode that throws `URIError` on truncated-percent input like `?next=%`. Concrete failure: user lands on `/login?next=%`, enters correct PIN → `verifyPin` succeeds and sets `session.user` → next synchronous line throws → catch displays "Could not verify PIN" → user is logged in but stranded on `/login`. Rev 16 removed the second decode and hardened `safeNext` with `try { decodeURI(u.pathname); } catch { return fallback; }` immediately after the `/login` short-circuit. `decodeURI` does NOT decode reserved characters (`%2F`, `%3F`, etc.), so legitimately-encoded paths still pass through; only malformed percent sequences trigger the fallback. New §6.2 router-test bullet locks `safeNext('%', origin, '/teaching') → '/teaching'`. (2) §4.1 dataflow shorthand updated from `navigate(safeNext(next))` to `safeNext(rawNext, origin, defaultLandingPath(user))`. NotFound `session.loading` concern from codex round 15 confirmed safe (`App.svelte:46-52` renders Spinner during session.loading, so NotFound never mounts in that window).
...
**Current state:** `RunDetailPage.svelte` passes `course` to `<RunAssetsTab>` (line 418, `course={course!}`). The other FOUR tabs (`<RunOverviewTab>`, `<RunMiniProjectsTab>`, `<RunTeachersTab>`, `<RunGroupsTab>`) currently receive NO `course` prop. Slice A threads `course` into all four to match the existing `RunAssetsTab` precedent — same prop shape across all five tabs. Additionally `<RunGroupsTab>` receives a new `runIsPublished` prop (see §3.2.4 RunGroupsTab block).
...
| `frontend/src/tests/RunMiniProjectsTab.svelte.test.ts` | 15 direct `mount(RunMiniProjectsTab, ...)` call sites (codex round-11 narrow grep). Extract a `mountMpTab(extra)` helper as part of this task so the prop change touches one helper, not 15 sites. |
| `frontend/src/tests/RunTeachersTab.svelte.test.ts` | 6 direct `mount(RunTeachersTab, ...)` call sites (codex round-11 narrow grep; inline mounts). |
...
- **`RunOverviewTab`** — accept required `course: Course` prop. Two changes:
  1. Wrap the `Delete run` button + delete confirm at `frontend/src/components/runs/RunOverviewTab.svelte:199-207` inside `{#if course.is_admin}`.
  2. **Groups-toggle tooltip rewrite** at line 161 — `title={run.is_published ? 'Locked once the run is published. Unpublish to change.' : ''}` tells a teacher to do something they cannot. Rewrite to be course-aware:
     ```svelte
     title={run.is_published
       ...
       : ''}
     ```
  PATCH-title / PATCH-end-date / metadata edits stay visible (PATCH on the run is `require_run_admin_or_teacher` per §3.1.6). The `groups_enabled` toggle stays visible and remains hard-disabled when `is_published` (same as today); only the tooltip becomes role-aware.
...
- **`RunGroupsTab`** (codex round-10 + 11) — accept TWO new required props: `course: Course` AND `runIsPublished: boolean`. **Important:** `runIsPublished` is NOT currently parent-supplied. The current parent passes only `runId`, `groups`, `groupsEnabled`, and callbacks at `RunDetailPage.svelte:369-375` — Slice A must extend that call site:
...
No new components. The hides are `{#if course.is_admin}` blocks; the prop wiring extends to `RunDetailPage` itself + 4 tabs (RunOverviewTab + RunMiniProjectsTab + RunTeachersTab + RunGroupsTab) + `MiniProjectModal` (for the admin-dead navigation CTAs — see next paragraph). The modal's Publish button stays visible to all roles per Option A.
...
**No second decode of `rawNext`.** `URLSearchParams.get` already decodes percent-encoded values per the WHATWG URL spec (`?next=%2F` → `/`, `?next=%252F` → `%2F`). The current `frontend/src/pages/Login.svelte:39` calls `decodeURIComponent(rawNext)` on top of that — a second decode that's redundant for valid encodings AND throws `URIError` on truncated-percent input (canonical example: `?next=%` — a `%` with no following hex digits survives URLSearchParams' lenient decoder verbatim, then throws inside `decodeURIComponent`). Incomplete UTF-8 sequences like `?next=%E0%A4` do NOT reach the decode — URLSearchParams substitutes a replacement character before rawNext is read, so they're handled silently regardless. If the throw fires, `verifyPin` has already succeeded (`session.user` is set) but the catch displays "Could not verify PIN" and the URL stays on `/login`, stranding a logged-in user on the login page. Rev 16 removes the second decode and lets `safeNext` parse the raw value with the lenient `URL` constructor + the new `decodeURI` guard below.
...
export function safeNext(next: string, origin: string, fallback = '/courses'): string {
  ...
  try {
    ...
    // Reject malformed percent-encoded sequences in the pathname
    // (e.g. '/%', '/%E0%A4'). The `URL` constructor is lenient and accepts
    // these without throwing, but routing/navigation downstream would then
    // hand a malformed URI to history.pushState. `decodeURI` throws URIError
    // on incomplete sequences; reserved characters (%2F, %3F, etc.) are
    // NOT decoded by `decodeURI`, so valid encoded paths pass through.
...
No call-site change required in `main.ts`. The existing `safeNext` unit tests in `frontend/src/tests/router.test.ts` continue to pass unchanged; new tests assert (a) `safeNext('/login?...', origin, '/teaching')` returns `/teaching`, and (b) `safeNext('%', origin, '/teaching')` returns `/teaching` (malformed-pathname guard).
...
Login.svelte post-PIN:
  - rawNext === null OR rawNext === '/' → navigate(defaultLandingPath(user))
  - rawNext is a real path              → navigate(safeNext(rawNext, origin,
...
**Disabled-version banner copy — course-aware** (`frontend/src/pages/runs/RunDetailPage.svelte:339-342`). Today's banner reads `"This run's course version is disabled. Re-enable it under Course Editor before publishing."` Both "Re-enable in Course Editor" (admin authoring) and "before publishing" (admin-only POST publish) are admin-dead for teachers. Slice A rewrites:
...
`RunDetailPage` already has `course` in scope. The `publishTooltip` at `RunDetailPage.svelte:231-233` is now inside the `{#if course.is_admin}`-wrapped `.publish-bar` (per §3.2.4), so it never renders for teachers — no rewrite needed there. Smoke step §6.3 step 6b verifies the teacher-facing banner copy.
...
Per §3.1.3, the teacher branch allows reads on disabled-version VERSION metadata and BLOCK content (text + markdown). Embedded asset `<img>` references inside `info_html` still 403 because `serve_asset` short-circuits on `is_disabled` BEFORE any role branch (per §3.1.3a + §5.4 admin-symmetric design). So §5.4 banner + tooltips render; embedded images on the disabled version appear as broken-image icons (same as admins see today). Test `test_blocks_list_allows_teacher_on_pinned_disabled_version` (§6.1) locks the blocks-read; `test_assets_serve_rejects_teacher_on_pinned_disabled_version` (§6.1) locks the admin-symmetric asset 403.
...
def test_verify_pin_response_includes_role_flags(
    client, admin_client, teacher_user, seed_publishable_version, db,
):
    # 1. Seed: teacher_user fixture (conftest.py:124-130) only creates a User row
    #    with NO RunTeacher row. For `has_run_teacher: True` we need a real
    #    RunTeacher row, so create a course + version + run via the admin client
    #    (the only path that satisfies require_course_admin_for_run), then insert
    #    the RunTeacher link directly via the db session.
    ...
    # 2. Request a PIN via the service layer (returns the raw PIN — same pattern
    #    used by backend/tests/conftest.py:107-141 fixtures; no separate helper).
    pin = request_pin(db, teacher_user.email)
    assert pin is not None  # rate-limit defensive check

    # 3. Verify via the HTTP route. `duration_days` is REQUIRED by
    #    PinVerifySchema at backend/mathion/schemas.py:169-172 — omitting it
    #    returns 422. Use 7 to match the conftest fixtures
    #    (`conftest.py:110, 119, 140, 195`).
    r = client.post(
        "/api/auth/verify-pin",
    ...
    # 4. Wrap preserved AND new flags present.
    assert "user" in body
    assert body["user"]["has_run_teacher"] is True
    assert body["user"]["has_course_admin"] is False

    # 5. Pre-existing fields still present (regression for schema-change risk).
...
- `safeNext('%', origin, '/teaching')` → `/teaching` (rev-16 malformed-pathname guard — `decodeURI('/%')` throws inside safeNext, returns fallback). Without this guard, a stranded-on-login bug surfaces when a user lands on `/login?next=%` and successfully authenticates.
...
- When `course.is_admin === false` AND `run.is_published === true`, the groups toggle's `title` attribute is `"Locked once the run is published. Ask a course admin to unpublish before changing."` (NOT the admin-facing "Unpublish to change." text).
- When `course.is_admin === true` AND `run.is_published === true`, the groups toggle's `title` attribute is `"Locked once the run is published. Unpublish to change."`
- When `run.is_published === false`, the groups toggle's `title` attribute is empty regardless of `course.is_admin`.
...
- When `course.is_admin === false` AND `pinnedVersion.is_disabled === true`, the disabled-version banner text is `"This run's course version is disabled. Some editing actions are locked until a course admin re-enables it."`
- When `course.is_admin === true` AND `pinnedVersion.is_disabled === true`, the disabled-version banner text is `"This run's course version is disabled. Re-enable it under Course Editor before publishing."`
...
- `frontend/src/pages/Login.svelte` — rewrite the post-PIN navigate at lines 36-39 to call `defaultLandingPath(user)` when `rawNext` is absent OR equals `/` (the App-side "decide by role" sentinel); for real `rawNext` values, call `safeNext(rawNext, location.origin, defaultLandingPath(user))` so a malformed/cross-origin/`/login` `next` falls back to the role-aware landing instead of `/courses`. **Do NOT call `decodeURIComponent(rawNext)` first** — `URLSearchParams.get` already decodes valid percent sequences, and a second decode throws `URIError` on truncated-percent input like `?next=%`, stranding logged-in users on `/login` (see §3.2.5 narrative). Capture the User returned from `verifyPin` (`lib/auth.svelte.ts:34`); extend the existing import at `Login.svelte:4` to include `defaultLandingPath` from `../lib/router.svelte` (the leading `../` is required — Login.svelte lives in `pages/`, not at the project root).
...
- `frontend/src/lib/router.svelte.ts` — add `defaultLandingPath(user)` helper AND widen `safeNext` signature with `fallback = '/courses'` defaulted parameter (replace each of the five hardcoded `'/courses'` returns at lines 196, 198, 201, 204, 207 with `fallback`); also add a `try { decodeURI(u.pathname); } catch { return fallback; }` guard immediately after the `/login` short-circuit (rev-16) so malformed percent-encoded paths fall back instead of leaking through `new URL`'s lenient parser into `history.pushState`. `main.ts:16` call site keeps default behavior; Login.svelte passes `defaultLandingPath(user)` as the fallback.
...
- `frontend/src/components/runs/RunMiniProjectsTab.svelte` — accept required `course: Course` prop; pass `course` to `MiniProjectModal`; add `const isLocked = (mp) => mp.first_submitted_at !== null` helper; for locked MPs AND `!course.is_admin`, hide the force-confirm Delete affordance; wrap the "Publish on Overview" link-button at line 185 in `{#if course.is_admin}`; gate "Enable on Overview" link-button at line 170-175 on `{#if !runIsPublished || course.is_admin}`; wrap "See Overview" link-button at line 179 in `{#if course.is_admin}`; rewrite the `newDisabledTitle` derivation at line 96-99 to be course/published-state aware. No MP-publish hide on the modal Publish button (teacher-allowed per §3.1.6).
...
- `frontend/src/tests/RunTeachersTab.svelte.test.ts` — extend (6 direct `mount()` sites per codex round-11 grep; inline mounts).
...
- `frontend/src/tests/RunDetailPage.svelte.test.ts` — extend (indirect; RunDetailPage now passes `course` to four more tabs AND `runIsPublished` to RunGroupsTab — verify integration).
...
8. Frontend: RunDetailPage prop-threading (incl. publish-bar split, breadcrumb rewrite, disabled-version banner copy split) + 4 tab prop updates (Overview / MP / Teachers / Groups) + modal prop threading + 6 component hide/copy blocks + test extensions across 6 test files.
