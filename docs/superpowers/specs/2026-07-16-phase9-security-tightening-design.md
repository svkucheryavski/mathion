...
**Status:** Codex-APPROVED (pass 3, after passes 1–2 CHANGES REQUIRED). All findings verified against code and folded in: scope expanded to 6 endpoints; fixture inventory 40/6 files; both GET unauthorized branches tested; inline 404 literals; orphaned import removed; timing residual acknowledged; byte-identity test; signature-screening wording. Awaiting final user approval, then writing-plans. First slice of the Phase 9 hardening arc.
...
The forbidden-path 404 is a plain inline `raise HTTPException(status_code=404, detail="Submission not found")` / `"Evaluation not found"` — the exact string `get_or_404` emits for that model (`f"{Model.__name__} not found"`). No shared helper (ten straightforward inline raises across the six endpoints — eight GET branches plus two mutation branches — with only two distinct messages; a helper would still have to pick the per-endpoint model name — YAGNI). The byte-identity test (see Testing) guards against any drift between the two strings.
...
- **Frontend:** the unauthorized branches are not reached in normal UI flows (the UI only requests the student's own group's data; evaluation create/patch are admin/teacher surfaces). No frontend code branches on `403` from these endpoints. Effect on legitimate users is nil in-flow; a stale or now-forbidden link (e.g. after group removal or run unpublish) returns `404` instead of `403` — the intended security behaviour, not literally zero user-visible change. A plan step confirms no client dependency on these 403s.
...
- Codex pass 3 (2026-07-16): **APPROVE with minor edits** — no Critical / no Important. Confirmed against code: both GET branch-site sets correct, run-unpublish fixture reaches the visibility branch (nothing membership-related short-circuits), inline details correct + drift-caught by the byte-identity test, import cleanup correct (exactly two `require_run_admin_or_teacher` refs removed; `is_run_admin_or_teacher` already imported; `submissions.py` needs no authz-import change), 6-endpoint scope complete, 40/6 fixture inventory exact, header-screen + timing-residual framing accurate. Two factual wording edits applied ("ten inline raises" not "six literals"; "run unpublish" not "version unpublish").
