...
- The `MINIPROJECT` critical section holds no ≤20 MB file I/O (file read/validate/temp-write precede the lock, §5.4); the pre-lock temp file is unlinked on **every** locked-region exit (gate/deadline 409s and errors) via a `try/finally`, proven by an orphan-`.upload-*.tmp` regression test.
