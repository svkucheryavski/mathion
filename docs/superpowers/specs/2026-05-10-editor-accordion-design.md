...
<div class="accordion-row">
  <button
    ...
    class="toggle"
  >
    <span class="title">{title || `(${level} ${index})`}</span>
...
- Toggle's accessible name comes from `labelFor(title, slug, positionalFallback)` via an explicit `aria-label` on the toggle button — this guarantees the SR announcement is non-empty even when `title` is blank during inline create-form authoring. The visible `<span class="title">` shows the title (or a parenthesised positional placeholder like `(block 3)` when empty) so the sighted user sees something. The visible `<span class="slug">` is `aria-hidden="true"` because screen readers would otherwise read `"/intro-1"` as `"slash intro dash one"`, doubling the announcement length without adding meaning. The aria-label takes precedence over the inner spans for accessible-name computation, so both the SR announcement and the visible text stay coherent across empty / partial / full title states.
