...
| **SequenceEditPage** `/.../sequences/:sequenceId` | reads from cached admin-tree | Edit `sequence.title` (`PATCH /api/sequences/{sid}`); list items with ↑/↓ + Open + per-row inline glyph keyed off `it.type` (does NOT reuse `ItemIcon.svelte`, which expects a full `Item` discriminated union and a click handler — slice 1 row uses an inline `<span>`); create item (icon-radio type picker, see below); "Delete this sequence" button (disabled when sequence has items or `!canEditStructure`). |
