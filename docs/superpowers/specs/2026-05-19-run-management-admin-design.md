...
**Date:** 2026-05-19
**Status:** Brainstorm-validated and reviewer-corrected (six rounds in-house + 3 independent codex passes); ready for implementation plan.
...
7. **`RunDetailPage` shell** — stale-guard with single commit gate + `loadError` placeholder + 401 filter (§3.2); `runId` string→int coercion with `Number.isInteger && > 0` validity guard; parallel fetch of 5 nested resources; tabs scaffold; **disabled-version banner derivation** (`$derived(versions?.find(v => v.id === run?.version_id))` with mid-load and miss fallbacks); `gotoTab(tab, prefilter?)` function + `onNavigateTab` callback prop passed to `RunOverviewTab`; `rosterPrefilter` state + `onPrefilterClear` callback prop passed to `RunRosterTab`; tab-state reset effect on `runId` change. Loading-state rendering uses `LoadingPlaceholder` from T3.
...
17. **`RosterImportModal` `buildBatchRow` (F1=A) + submit + result** — **on Import click**: invoke `onRefetchBeforeSubmit()` callback prop to get fresh `students` AND `groups` (narrows the rename race window for F1=A name resolution; see §4.5). Then `buildBatchRow(parsedRow, freshRoster, freshGroups)` with the 4-case logic (F1=A: already-enrolled + empty cell + has group → resolve current group name; already-enrolled + empty cell + null group → omit `group`; brand-new + empty cell → omit `group`; non-empty cell sent as-is); `POST /api/runs/{runId}/students/batch`; stage-2 result table with mixed success/error — error rows render the `result.detail` string verbatim (backend message) and surface it as a `title` tooltip on the row's `error` badge (`RunStudentBatchResultRow` has no `error_code`); `Copy failed rows` with `navigator.clipboard.writeText` + textarea fallback on `DOMException`; `Done` → parent refetches `students` AND `groups`. Tests assert all 4 wire-shape cases including `Object.prototype.hasOwnProperty('group')` absence assertion (not just `=== undefined`); plus a test for the submit-time refetch (mock the callback, assert it's called once between Import click and POST).
