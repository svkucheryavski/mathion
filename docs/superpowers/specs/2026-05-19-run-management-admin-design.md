...
**Date:** 2026-05-19
**Status:** APPROVED FOR PLAN-WRITING — Brainstorm-validated and reviewer-corrected (six rounds in-house + 5 independent codex passes; R11 verdict: APPROVED).
...
>
> **R9 (post-third-codex-review):** Third codex pass verified R8 fixes are correct and ran a final 10-point sweep. Nine of the 10 sweep points returned N/A; one (#7) surfaced a real concurrent-rename race in the F1=A `buildBatchRow` path. **Fix:** `RosterImportModal` now invokes a new `onRefetchBeforeSubmit()` callback prop immediately before computing batch rows, getting fresh `groups` AND `students` from the parent. This narrows the race window (between modal open and POST) to a single network round-trip; without it, a concurrent group rename in another tab caused the frontend to send the stale name, triggering backend auto-create of a duplicate group with the old name (silent data corruption). True elimination would require backend to accept `group_id` instead of `group` name on the batch endpoint — non-goal for v1 (added to §8 as known limitation). T17 description updated to mention the refetch step and adds a test bullet for it.
>
> **R10 (post-fourth-codex-review):** Fourth codex pass verified R9's intent is correct but flagged R9's edit was under-specified in three ways. All three closed: (1) **`onRefetchBeforeSubmit` is now required, not optional** — the `?` was inconsistent with the unconditional invocation in T17, and an "optional with silent fallback" would re-expose the rename race; the F1=A correctness contract demands the refetch. (2) **Callback chain named end-to-end:** `RunDetailPage` exposes `refetchRosterData(): Promise<{students, groups}>` (in-band refetch — no `loadToken` bump; writes both slices directly, which auto-triggers `prunePendingGroups()` via the §4.4 prune-on-students-assign rule); passes to `RunRosterTab` as **required** `onRefetchRosterData` prop; `RunRosterTab` forwards as `onRefetchBeforeSubmit` to the modal. §3.5 and §10 T7 updated. (3) **§4.4 prune-paths enumeration** now lists the submit-time refetch as path #5 (was previously implicit). Plus two cosmetic clarifications: T17 explicitly specifies the Import button disabled/loading state across the refetch + POST chain, and the test bullets enumerate every assertion (callback called once, button disabled during in-flight, Cancel/Escape no-op during submit).
>
> **R11 (post-fifth-codex-review):** Fifth codex pass verified all R10 fixes hold cleanly. Sweep verdict: APPROVED FOR PLAN-WRITING. One Minor cosmetic gap absorbed: §7 `RunDetailPage` test list now explicitly asserts the `refetchRosterData()` in-band contract — parallel awaits, direct slice assignment, no `loadToken` bump, returns the fresh slices to the caller. No new findings; spec has converged at 11 rounds (6 in-house + 5 codex).
...
frontend/src/components/ui/                    # shared primitives, used 2+ sites
  InlineConfirm.svelte        # morph: button → [Confirm] [Cancel] pair
  LoadingPlaceholder.svelte   # muted "Loading…" + optional spinner glyph
  FocusTrap.svelte            # modal Tab-trap + previous-focus capture + tolerant restore
...
frontend/src/tests/
  ...
  runStatus.test.ts
  csv.test.ts
  InlineConfirm.svelte.test.ts
  LoadingPlaceholder.svelte.test.ts
  FocusTrap.svelte.test.ts
...
async function loadAll(slug: string, rid: number) {
  ...
  try {
    ...
    // redirect). We do NOT surface a loadError placeholder in that case —
    // doing so would briefly render an error screen before the redirect
    // takes the user away. Filter explicitly.
...
- Right: status badge + `Publish` / `Unpublish` button.
  ...
  - When `is_published`: button reads `Unpublish`, always enabled. Click renders the `InlineConfirm` (from §3.1) with `label="Unpublish"`, `confirmLabel="Confirm Unpublish"`, and `warning="Students will lose access immediately. Their progress data is preserved."`. After confirm-and-success, re-fetch `run`.
...
- `RunDetailPage` passes `gotoTab` to `RunOverviewTab` via a callback prop named `onNavigateTab` (parallel to `onPrefilterClear` below). `RunOverviewTab` props declare: `onNavigateTab: (tab: ActiveTab, prefilter?: 'unassigned' | null) => void`. The "N unassigned" hint button calls `onNavigateTab('roster', 'unassigned')`. The single-tick atomicity is structural (one synchronous function call sets both); no separate `pendingTab` state required.
- `rosterPrefilter` is cleared by `RunRosterTab` itself when the user types in the search box or clicks the prefilter pill's `×` (see §4.4) — assigning back to the parent's `$state` via a callback prop `onPrefilterClear={() => rosterPrefilter = null}`. `RunRosterTab` props declare: `onPrefilterClear: () => void`.
- `RunDetailPage` also exposes `refetchRosterData()` (defined adjacent to `loadAll`; see §4.5) and passes it as the **required** `onRefetchRosterData` callback prop on `RunRosterTab`. `RunRosterTab` props declare: `onRefetchRosterData: () => Promise<{ students: RunStudentResponse[]; groups: GroupResponse[] }>`. `RunRosterTab` forwards it to `RosterImportModal` as `onRefetchBeforeSubmit` for the F1=A submit-time freshness (§4.5).
...
| All students assigned to a group | If `!run.groups_enabled`, render `—`. Otherwise `✓` if every `student.group_id !== null`, else `✗` with hint button "N students unassigned" — clicking invokes the `onNavigateTab('roster', 'unassigned')` callback prop on the parent (`RunDetailPage`). |
...
4. **`RunGroupsTab` group-delete 409 "Group has students; reassign or remove first" race** (§4.3, refetches `groups` AND `students` — line ~482).
5. **`RosterImportModal` submit-time refetch** (§4.5, via the new `onRefetchBeforeSubmit` chain — runs immediately before `buildBatchRow`; assigns fresh `students` + `groups` to the parent's `$state` and therefore triggers `prunePendingGroups()`).
...
- On select change for `user_id=U` to target `G` (`null` or `int`):
  ...
  - On 409 `capacity_reached`: `pendingGroupId.delete(U)`; toast `"Target group is full (10 students)."` (matches the message used by inline-add and bulk-op tooltip — single canonical phrasing).
...
This is computed at submit time (Stage 1 → Stage 2 transition), not in the preview. The preview shows the parsed cell verbatim; the lookup happens only for rows actually submitted.

**Submit-time freshness — minimize the rename race.** The F1=A lookup uses the cached `groups`/`existingRoster` arrays. Backend `run_roster.py:140-147` auto-creates a group when `row.group` doesn't match an existing name. If another admin renames a group between modal open and submit, a stale cache would cause the frontend to send the OLD group name → backend can't find it → auto-creates a duplicate group with the old name (silent data corruption). To minimize this race, immediately before running `buildBatchRow` on the valid CSV rows, **refetch `groups` AND `students`** from the parent. This narrows the race window to a single network round-trip; a true fix (backend accepts `group_id` instead of `group` name) is non-goal for v1 — listed in §8.

**Callback chain (parent supplier identity).** Because `students` and `groups` are owned by `RunDetailPage` (§3.2), the refetch must be triggered there. The chain is:

- `RunDetailPage` exposes a `refetchRosterData(): Promise<{ students: RunStudentResponse[]; groups: GroupResponse[] }>` function that re-fetches both slices and assigns them to the parent's `$state` (in-band refetch contract per §3.2 — does NOT bump `loadToken`; writes the two slices directly). After commit, the assignment of `students` automatically runs `prunePendingGroups()` (see §4.4 prune rule) — covers the submit-time refetch path.
- `RunDetailPage` passes this function to `RunRosterTab` as a callback prop. `RunRosterTab` props declare: `onRefetchRosterData: () => Promise<{ students: RunStudentResponse[]; groups: GroupResponse[] }>` (required, not optional).
- `RunRosterTab` passes it through to `RosterImportModal` as the `onRefetchBeforeSubmit` prop. `RosterImportModal` props declare: `onRefetchBeforeSubmit: () => Promise<{ students: RunStudentResponse[]; groups: GroupResponse[] }>` (**required**, not optional — the F1=A correctness depends on it; making it optional with a silent fallback would re-expose the rename race).

**Import button loading state during refetch + POST.** Per the global convention (§3.2 "Buttons that trigger fetches show a disabled state with a small spinner glyph / `…` suffix"), the `Import N valid rows` button is disabled and shows `Importing…` from the moment of click through (a) the `onRefetchBeforeSubmit` resolution AND (b) the subsequent `POST /api/runs/{runId}/students/batch` resolution. Both Cancel and Escape are also disabled in this window — submit is committed once Import is clicked.

**Other race tradeoffs (acknowledged, not blocking):**
- If the resolved group has since become **disabled** (race with `is_disabled` toggle in another tab), the backend's per-row 409 surfaces in Stage 2 — the teacher can resolve it manually.
- If the run's pinned version has become disabled, the entire submit fails with 403 (§6 handler).
- If the rename race happens DURING the network round-trip from refetch to POST (~milliseconds), the duplicate-group symptom can still occur. Documented as a known limitation; not worth additional plumbing in v1.
...
**Helper patterns established for this feature** (set up in T1–T4 — `lib/runs.ts` helpers, shared UI primitives, pure-function modules — and reused by later tasks):
...
- **DST test timezone pin.** `tests/runStatus.test.ts` pins `process.env.TZ = 'America/New_York'` in a `beforeAll`/`afterAll` (capture original, restore) so the spring-forward DST boundary check around `2026-03-08` is deterministic regardless of CI runner timezone. (The runStatus helper uses local-time semantics by design; pinning TZ is the only way to make the DST test reproducible.) **Fallback if `process.env.TZ` does not take effect** (some jsdom worker setups cache locale data before the test bootstrap mutates `process.env`): construct the `now` Date with an explicit offset string, e.g., `new Date('2026-03-08T02:30:00-05:00')` (EST) and `new Date('2026-03-08T03:30:00-04:00')` (EDT), and assert behavior across both. Either approach satisfies the regression intent; tests should not silently degrade if the env-pin no-ops.
...
- **True elimination of the F1=A rename race in roster import.** The batch endpoint accepts `group: str | None` (a name, not an id); if a group is renamed in another tab during the modal's open-to-submit window, the backend may auto-create a duplicate group with the old name. R9 mitigation: refetch immediately before submit narrows the window to a single round-trip. A real fix requires backend to accept `group_id` on the batch endpoint — deferred.
...
3. All eight new run-management Svelte components plus three shared UI primitives (`InlineConfirm`, `LoadingPlaceholder`, `FocusTrap`) compile, render, and pass vitest tests.
...
9. The **18-task** implementation plan completes with each task individually verified and reviewer-approved.

---

## 10. Task decomposition preview

The implementation plan (written next via `superpowers:writing-plans`) will use **18 tasks** following the R4 rebalance (each right-sized for a single subagent work cycle):

1. **`lib/runs.ts` + types in `lib/types.ts`** — backend-mirror type block (`RunResponse`, `GroupResponse`, etc., per §5.1); 7 run-CRUD helpers; tests. Verify backend response shapes match.
...
4. **`lib/runStatus.ts` + `lib/csv.ts` + tests** — pure functions. DST test pins TZ to `America/New_York` with explicit-offset Date fallback (§7). Establishes fake-timer + clipboard-mock patterns reusable by later tasks.

5. **`RunListPage` + routes + `App.svelte` componentMap + `CourseCard` "Runs" button** — `routes.ts` adds two rows; `App.svelte:14-25` componentMap registers both new pages; list table rendered in backend order (`start_date ASC` — **frontend does NOT re-sort**) with status badge + version label resolution + delete-only-when-draft; loading state uses `LoadingPlaceholder` from T3; **"No runs yet" empty-state with `Create the first run` CTA** that opens `NewRunModal` (T6); admin-gate 403 → redirect via `router.navigate`; 404 → inline "Course not found" with link to `/courses`; version-empty disabled state on New-run button (with tooltip "Publish a course version before creating a run."); `CourseCard` admin-only branch restructured to mirror mixed-admin pattern (card-as-div, title-as-`<a>`, sibling `Edit` and `Runs` buttons gated on `course.is_admin`).

6. **`NewRunModal`** — modal scaffold consuming `FocusTrap` from T3; 4 validation tests (title empty, start empty, end empty, end < start); submit payload omits `version_id`; success → close + navigate (tolerant focus restore via `FocusTrap`'s `isConnected` guard); API error → banner with `e.displayMessage`. Establishes the modal pattern reusable by T16.

7. **`RunDetailPage` shell** — stale-guard with single commit gate + `loadError` placeholder + 401 filter (§3.2); `runId` string→int coercion with `Number.isInteger && > 0` validity guard; parallel fetch of 5 nested resources; tabs scaffold; **disabled-version banner derivation** (`$derived(versions?.find(v => v.id === run?.version_id))` with mid-load and miss fallbacks); `gotoTab(tab, prefilter?)` function + `onNavigateTab` callback prop passed to `RunOverviewTab`; `rosterPrefilter` state + `onPrefilterClear` callback prop passed to `RunRosterTab`; **`refetchRosterData()` in-band refetch helper** (re-fetches `students` AND `groups`, assigns them directly without bumping `loadToken`) passed as `onRefetchRosterData` callback prop on `RunRosterTab`; tab-state reset effect on `runId` change. Loading-state rendering uses `LoadingPlaceholder` from T3.

8. **Sticky publish bar** (lives at the top of `RunDetailPage`) — Publish/Unpublish buttons; **shared readiness derivation lives in `RunDetailPage`** as a single `$derived` (returns `{ checks: ChecklistRow[], firstViolation: string | null }`) consumed by both the publish-button tooltip (this task: shows `firstViolation`) AND the Overview tab's full checklist (T10: renders `checks`). Publish 409 banner under the button; Unpublish via `InlineConfirm` from T3 with the warning string (see §3.5); post-success run refetch.

9. **`RunOverviewTab` inline edits with `makeDirtyTracker`** — one bundled tracker for `{ title, start_date, end_date }` initialized via `$effect` when `run` first becomes non-null (§4.1); per-field commit on blur with cross-field revert rule (compare `tracker.current[field]` with captured `inFlightValue`); Enter/Escape/blur semantics including "blur after Enter does not double-PATCH"; jsdom date-input `.value` string helper. Calls `pushToast` from `stores/toasts.svelte.ts` on error.

10. **`RunOverviewTab` checklist + settings + danger zone** — tri-state readiness checklist with 7 permutations (§4.1 (c)) — **consumes the `checks` array from the parent's shared `$derived` (introduced in T8)**, does not duplicate the readiness computation; settings panel with groups_enabled toggle (publish-lock tooltip + mini-projects-409 fallback); danger zone with `InlineConfirm` from T3 for Delete + 204 → navigate back + 2× 409 toasts (`"Clear roster before deleting."` / backend verbatim for "submissions"). The "N unassigned" hint button invokes `onNavigateTab('roster', 'unassigned')` on the parent.
...
15. **`RunRosterTab` bulk ops banner + retry** — consumes the `bulkOpResult` `$state` + `dispatchBulkOp` reference from T14; renders the three banner shapes based on `bulkOpResult.kind` (full success / per-row partial / chunk-level cancellation); auto-dismiss 5s only on full success (`kind === 'success'`); retry control invokes `dispatchBulkOp(...)` with `cancelledIds ∪ chunkErrorRowIds` (so retry of >200 rows re-chunks correctly via the dispatcher); bulk-move whole-call 400/409 treated as chunk-level failure with `Retry cancelled → group [▼]` (different group, since the original target was bad); banner non-dismissible during in-flight retry (`kind === 'in-flight'`).

16. **`RosterImportModal` scaffold** — two-stage modal (paste & preview, then result); `FocusTrap` from T3; debounced live-parse (200ms with cancellation on rapid keystrokes); preview table (max ~10 rows visible, scrollable) with `#`/`Name`/`Email`/`Group`/`Status` columns; counts footer with `, +N more` truncation on already-enrolled list; `Cancel` / `Import N valid rows` buttons (disabled when 0 valid). No submit logic yet.

17. **`RosterImportModal` `buildBatchRow` (F1=A) + submit + result** — **Props (per §4.5):** `onRefetchBeforeSubmit: () => Promise<{ students, groups }>` (**required**, supplied by `RunRosterTab` from `RunDetailPage`'s `refetchRosterData`; commits to parent state via the in-band-refetch contract, which auto-triggers `prunePendingGroups()`). **On Import click:** disable Import button + render `Importing…`, disable Cancel and Escape; await `onRefetchBeforeSubmit()` to get fresh `students` AND `groups`; pass those into `buildBatchRow(parsedRow, freshRoster, freshGroups)` with the 4-case logic (F1=A: already-enrolled + empty cell + has group → resolve current group name; already-enrolled + empty cell + null group → omit `group`; brand-new + empty cell → omit `group`; non-empty cell sent as-is); `POST /api/runs/{runId}/students/batch`; re-enable controls when POST resolves and Stage 2 renders. Stage-2 result table with mixed success/error — error rows render the `result.detail` string verbatim (backend message) and surface it as a `title` tooltip on the row's `error` badge (`RunStudentBatchResultRow` has no `error_code`); `Copy failed rows` with `navigator.clipboard.writeText` + textarea fallback on `DOMException`; `Done` → parent runs the same `refetchRosterData()` (no second `prunePendingGroups()` call needed beyond the one the setter wraps). **Tests:** assert all 4 wire-shape cases including `Object.prototype.hasOwnProperty('group')` absence assertion (not just `=== undefined`); assert `onRefetchBeforeSubmit` is called exactly once between Import click and POST (mock callback); assert Import button is disabled while the promise chain runs; assert Cancel/Escape are no-ops during in-flight submit.

18. **Final integration** — 24-step manual smoke walk-through with explicit pass criterion (all 24 steps green; backend-side issues filed as follow-ups rather than blocking merge); full vitest run with **count delta** vs main; `svelte-check` baseline comparison (warnings unchanged); branch cleanup; merge prep.
