...
Under `MATHION_DEBUG` with email **off**, `email_enabled` is still `false` (the flag keys purely on `email_mode`), so the login screen is the **direct email+PIN** form — which does not call `request_pin`, so the console print (`auth.py:74`) is **unreachable from that screen**. The debug developer obtains the PIN via the CLI `pin <email>` (which prints unconditionally), exactly as in the production bootstrap. `MATHION_DEBUG` is dev-only and is not paired with a live mailer in an environment with real users.
...
**Email-aware login (bootstrap enabler):** `Login.svelte` fetches `GET /api/auth/config` (§5) on mount. When `email_enabled` is **false**, it skips the "Send PIN" step entirely and presents a combined **email + PIN** form with an explanatory message ("Email delivery isn't configured — enter your email and the PIN shown in the server terminal"), submitting straight to `verify-pin` **without** calling `request-pin`. When `email_enabled` is **true**, the two-step flow (email → Send PIN → PIN) is used and the PIN is now actually delivered to the user's inbox (§5 "Login-PIN delivery"); because the send is synchronous, the "Send PIN" button shows a **pending state** while the request is in flight (worst case ~30 s on a slow SMTP host, §5) so the wait is not read as a frozen UI. **Until `/api/auth/config` resolves, Login renders neither form** — it shows a loading state and picks the form shape only after `email_enabled` is known (a *render-gate*, mirroring the whole-subtree block of the app's `session.loading` gate at `App.svelte:66` — NOT merely a disabled button, which an Enter key can bypass). `onSubmitEmail` additionally early-returns while config is unresolved (`if (emailEnabled === undefined) return;`) as belt-and-suspenders — otherwise a "Send PIN" before config lands would fire the very `request_pin` this avoids. **If the config fetch fails (network/5xx), resolve the gate into the standard two-step (email-enabled) flow** (set the resolved state in the `catch` so submit re-enables — no infinite spinner) — the production-normal path; if email really is off, the operator just re-runs `pin`. This is what makes the terminal-printed bootstrap PIN usable (§6) — the disabled-email screen never fires the PIN-invalidating `request_pin`. The adaptation is not superuser-specific: with email off, no user can receive a PIN, so direct entry is the correct behavior for everyone until email is configured.
...
- `pages/Login.svelte` — capture + clear `sessionStorage['superuser_return_path']` **synchronously at init / first line of `onMount`, before any `await`** (precedence over `?next=` at successful verification); fetch `GET /api/auth/config` on mount and **render-gate** (render neither form until it resolves), then when `email_enabled=false` present the combined email+PIN form (skip "Send PIN"), with an `onSubmitEmail` early-return while config is unresolved; show a pending state on "Send PIN" during the synchronous send.
- `lib/auth.svelte.ts` — add `getAuthConfig() -> { email_enabled }` (used by Login); **no `skipAuthRedirect` needed** — the endpoint is public and never 401s, so the flag would be inert (do not rely on it swallowing errors).
