...
- **PIN-delivery-aware login:** a public `GET /api/auth/config` → `{ send_pin_enabled }` + `Login.svelte` showing the normal two-step "Send PIN" flow whenever a Send-PIN would deliver a retrievable PIN (a delivering email mode — `smtp`/`file` — or the `MATHION_DEBUG` console), or direct email+PIN entry where it would not (a `disabled`/`memory` or otherwise fresh server → operator uses the CLI-generated PIN).
...
`GET /api/auth/config` — **unauthenticated**, no token, no CSRF (GET). Returns `{ send_pin_enabled: bool }` where **`send_pin_enabled = settings.debug or settings.email_mode in ("smtp", "file")`** — true whenever clicking "Send PIN" yields a PIN the requester can actually retrieve. Three retrievable cases: `MATHION_DEBUG` (the two-step flow's `request_pin` prints to the server console, `auth.py:74` — which is why `debug` is folded in, and why this is **not** codex's `not debug and …`: it preserves today's dev login, Send PIN → console, rather than forcing direct-entry+CLI in debug); `email_mode=smtp` (the user's inbox); `email_mode=file` (a persisted `.eml` in the operator's outbox). It **excludes** `email_mode=memory` even though that is non-`disabled`: the login-PIN send builds a **one-shot** mailer (below) and `MemoryMailer` only appends to its in-process `sent` list (`mailer.py:97`), so the per-send instance is discarded and the PIN is retrievable by no one — `memory` is a test-only sink, and a bare `email_mode != "disabled"` flag would dishonestly show the two-step "check your inbox" flow there. Exposing this operational boolean is not sensitive: `false` reveals only "not debug and no deliverable mailer" (a fresh/unprovisioned, or `memory`/test, server); `true` does not distinguish debug from a configured mailer. Lives in `api/auth.py` (a general auth concern, not superuser-specific), in scope for this slice because the bootstrap depends on it.
...
|-----------|-------------------|
| `MATHION_DEBUG=1` (any `email_mode`) | server console (unchanged); **no** email |
| debug off **+** `email_mode` ∈ {`smtp`, `file`} | **sent synchronously** by the `/api/auth/request-pin` endpoint — `smtp` → the user's inbox, `file` → an `.eml` in the operator's outbox (retrievable) |
| debug off **+** `email_mode=memory` | the endpoint still attempts a send (so the §9 test can inject an observable `MemoryMailer`), but a **one-shot** `MemoryMailer` is discarded with the request — a **test-only sink, not retrievable**; `send_pin_enabled=false`, so the UI shows direct-entry (this endpoint-sends-but-flag-is-false asymmetry is intentional) |
| debug off **+** `email_mode=disabled` | not delivered — obtainable only via the CLI `pin` (bootstrap) |
...
- `GET /api/auth/config` returns `send_pin_enabled` over `debug` × the four `email_mode` values: with `debug` **off**, `true` for `smtp`/`file` and **false** for `memory` and `disabled` (a one-shot `MemoryMailer` delivers nowhere retrievable); with `debug` **on**, `true` for **every** `email_mode` (console print). Explicitly assert the **`debug` off + `email_mode=memory` → false** case and the **`debug` on + `email_mode=disabled` → true** case; no auth required.
...
- **PIN-delivery-aware login:** with `send_pin_enabled=false` the login screen skips "Send PIN" and submits email+PIN directly to `verify-pin` (no `request-pin` call); with `send_pin_enabled=true` the two-step flow is unchanged and its success copy is **delivery-neutral** — assert the rendered `pin`-step text does **not** claim email-only delivery (no "sent to {email}" / "to your inbox" wording) **and** does contain a channel-neutral phrase (e.g. "a 6-digit PIN has been sent" / "check your email or the server console"), so a later revert to a different email-only phrasing is still caught; while config is in flight **neither form is rendered** (render-gate) and mounting with an unresolved config fetch fires **no** `request-pin` even on an Enter/submit attempt; on a config-fetch failure Login resolves into the two-step flow **and submit becomes enabled** (no infinite spinner).
...
- `pages/Login.svelte` — capture + clear `sessionStorage['superuser_return_path']` **synchronously at init / first line of `onMount`, before any `await`** (precedence over `?next=` at successful verification); fetch `GET /api/auth/config` on mount and **render-gate** (render neither form until it resolves), then when `send_pin_enabled=false` present the combined email+PIN form (skip "Send PIN"), with an `onSubmitEmail` early-return while config is unresolved; show a pending state on "Send PIN" during the synchronous send; and **replace the hardcoded two-step success copy** ("A 6-digit PIN was sent to {email}", `Login.svelte:64`) with delivery-neutral wording (§7).
