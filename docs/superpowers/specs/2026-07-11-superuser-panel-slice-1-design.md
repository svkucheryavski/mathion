...
- **Email-aware login:** a public `GET /api/auth/config` → `{ email_enabled }` + `Login.svelte` showing the normal two-step "Send PIN" flow when email is configured, or direct email+PIN entry when it is not (fresh server → operator uses the CLI-generated PIN).
...
The email send is **synchronous** (in the endpoint, via `request.app.state.mailer`), NOT through the `NotificationLogEntry` queue, for two reasons: (1) login PINs are latency-critical — the dispatcher batches on a tick, so a queued PIN could arrive minutes late; (2) the queue persists its payload in `notification_log`, which would put the **raw PIN in the DB in plaintext** — PINs are otherwise only stored hashed. The raw PIN is built into an `EmailMessage` in memory (`X-Mathion-Kind: login_pin`) and sent inside a one-shot `mailer.session()` (SMTPMailer opens/closes a connection per send — fine at login frequency); it is **never persisted or logged**. A send failure (e.g. SMTP down) surfaces an error to the caller (UI shows "couldn't send PIN, try again") rather than a false "PIN sent". Delivery lives in the endpoint, not in `request_pin` itself, so the CLI `pin` path (operator bootstrap) never emails.
...
**Email-aware login (bootstrap enabler):** `Login.svelte` fetches `GET /api/auth/config` (§5) on mount. When `email_enabled` is **false**, it skips the "Send PIN" step entirely and presents a combined **email + PIN** form with an explanatory message ("Email delivery isn't configured — enter your email and the PIN shown in the server terminal"), submitting straight to `verify-pin` **without** calling `request-pin`. When `email_enabled` is **true**, the two-step flow (email → Send PIN → PIN) is used and the PIN is now actually delivered to the user's inbox (§5 "Login-PIN delivery"). **Until `/api/auth/config` resolves, Login shows a brief loading state (or disables submit) and picks the form shape only after `email_enabled` is known** — otherwise a "Send PIN" click before config lands would fire the very `request_pin` this avoids (analogous to the app's existing `session.loading` gate, `App.svelte:66`). **If the config fetch fails (network/5xx), fall back to the standard two-step (email-enabled) flow** — the production-normal path; if email really is off, the operator just re-runs `pin`. This is what makes the terminal-printed bootstrap PIN usable (§6) — the disabled-email screen never fires the PIN-invalidating `request_pin`. The adaptation is not superuser-specific: with email off, no user can receive a PIN, so direct entry is the correct behavior for everyone until email is configured.
...
- Login-PIN delivery: under `MATHION_DEBUG` the PIN is console-printed and **not** emailed; with debug off + a `MemoryMailer`, `/api/auth/request-pin` sends **exactly one** `login_pin` message containing a PIN that `verify_pin` accepts, and **no raw PIN is written to `notification_log`**; with debug off + no mailer, nothing is sent; a mailer send failure surfaces an error (not a false success).
...
- **Email-aware login:** with `email_enabled=false` the login screen skips "Send PIN" and submits email+PIN directly to `verify-pin` (no `request-pin` call); with `email_enabled=true` the two-step flow is unchanged; while config is in flight submit is gated (no "Send PIN" fires); on a config-fetch failure Login falls back to the two-step flow.
...
- `api/auth.py` — (1) add public `GET /api/auth/config` → `{ email_enabled }`; (2) in `/request-pin`, after `request_pin`, when `not settings.debug and request.app.state.mailer is not None`, build a `login_pin` `EmailMessage` and send it synchronously (`with mailer.session(): mailer.send(msg)`), surfacing send failures as an error and never persisting/logging the raw PIN; (3) wire the logout hook to destroy the active panel token only when the logging-out session's user `is_superuser`.
- `notifications/templates.py` (or a small auth-side helper) — a `login_pin` message builder (subject + body carrying the code; a plain `(email, pin) -> EmailMessage`, NOT the Run-coupled `RenderContext`, and NOT added to the `TEMPLATES` dict — those are `RenderContext → (subject, body)` functions consumed by `render()`, a different contract). Since `FileMailer._allowed_kinds` is derived from `TEMPLATES.keys()` (`mailer.py:43,48`), union `{"login_pin"}` into it so file-mode names the `.eml` `login_pin` rather than `unknown` (cosmetic — the MemoryMailer used by the delivery test in §9 is unaffected).
