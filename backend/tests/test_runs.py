def test_create_run_pins_to_newest_published_version(admin_client, db, seed_publishable_version):
...
def test_create_run_end_before_start_422(admin_client, db, seed_publishable_version):
...
def test_list_runs(admin_client, db, seed_publishable_version):
    course, _ = seed_publishable_version()
...
def test_get_run(admin_client, db, seed_publishable_version):
    course, _ = seed_publishable_version()
...
def test_patch_run_title(admin_client, db, seed_publishable_version):
    course, _ = seed_publishable_version()
    run = admin_client.post(f"/api/courses/{course['id']}/runs",
...
def test_patch_run_version_id_ignored(admin_client, db, seed_publishable_version):
...
def test_delete_unpublished_run(admin_client, db, seed_publishable_version):
    course, _ = seed_publishable_version()
    run = admin_client.post(f"/api/courses/{course['id']}/runs",
        json={"title": "R", "start_date": "2026-01-01", "end_date": "2026-06-01"}).json()
...
def test_non_admin_cannot_create_run(auth_client, admin_client, db, seed_publishable_version):
    course, _ = seed_publishable_version()
